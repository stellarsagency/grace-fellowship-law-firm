(function(){
  ...
  css.textContent='#chatWidget{position:fixed;bottom:24px;right:24px;z-index:9999;font-family:"Source Sans 3",sans-serif}'
    +'#chatFab{width:68px;height:68px;border-radius:50%;background:#428475;color:#fff;border:none;font-size:1.8rem;cursor:pointer;box-shadow:0 6px 24px rgba(66,132,117,.4);transition:all .3s;display:flex;align-items:center;justify-content:center}'
    ...
    +'.cb-head{background:#1A312C;padding:14px 16px;display:flex;align-items:center;gap:12px;flex-shrink:0}'
    +'.cb-head img{width:36px;height:36px;border-radius:50%;border:2px solid #428475}'
    +'.cb-head-text h4{font-family:"Playfair Display",serif;color:#fff;font-size:.92rem;margin:0}'
    +'.cb-head-text span{font-size:.7rem;color:#5aa18f;display:flex;align-items:center;gap:4px}'
    ...
    +'.cb-user{background:#1A312C;color:#fff;align-self:flex-end;border-bottom-right-radius:2px}'
    +'.cb-bot strong{color:#1A312C;display:block;margin-bottom:3px;font-size:.82rem}'
    +'.cb-bot a{color:#428475;font-weight:700;text-decoration:underline}'
    ...
    +'.cb-input button{width:44px;height:44px;border-radius:50%;background:#428475;color:#fff;border:none;font-size:1.2rem;cursor:pointer;transition:all .3s;flex-shrink:0;display:flex;align-items:center;justify-content:center}'
    +'.cb-input button:hover{background:#1A312C;color:#fff}'
    +'.cb-typing{display:flex;gap:4px;padding:10px 14px;align-self:flex-start}'
    +'.cb-typing span{width:7px;height:7px;background:#428475;border-radius:50%;animation:cbTyp 1.4s infinite}'
    +'.cb-typing span:nth-child(2){animation-delay:.2s}'
    +'.cb-typing span:nth-child(3){animation-delay:.4s}'
    +'@keyframes cbTyp{0%,60%,100%{transform:translateY(0);opacity:.4}30%{transform:translateY(-5px);opacity:1}}'
    +'@media(max-width:480px){#chatBox{right:10px;bottom:80px;width:340px;max-width:calc(100vw - 20px);height:480px;max-height:calc(100vh - 140px);border-radius:12px}#chatFab{width:56px;height:56px;font-size:1.5rem;bottom:16px;right:16px}.cb-head{padding:10px 12px}.cb-head img{width:30px;height:30px}.cb-head-text h4{font-size:.82rem}.cb-msgs{padding:10px}.cb-msg{font-size:.82rem;max-width:88%}.cb-input{padding:8px 10px}.cb-input input{padding:8px 12px;font-size:.82rem}.cb-input button{width:38px;height:38px;font-size:1rem}.cb-qr{padding:0 10px 8px}.cb-qr-btn{padding:5px 10px;font-size:.72rem}}';
...
- Always mention services are FREE when discussing legal help`;
  ...
  w.innerHTML='<button id="chatFab" aria-label="Chat with us">&#128172;</button>'
    ...
    +'<div class="cb-msgs" id="cbMsgs"><div class="cb-msg cb-bot"><strong>aSK peter Charles</strong>Welcome to Grace Fellowship Law Firm. We are here to assist you. If you have any question, you can ask me for help. Below are our contact options:<br><br><a class=\'cb-contact-btn\' href=\'https://wa.me/923004287689\' target=\'_blank\'>&#128241; WhatsApp</a> <a class=\'cb-contact-btn\' href=\'mailto:gflf.official@gmail.com\'>&#9993; Email</a></div></div>'
